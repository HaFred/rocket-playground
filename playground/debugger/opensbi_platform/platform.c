 ...
 * See config.mk ROCKETCHIP_xxx configuration parameters.
 */
#include <sbi_utils/irqchip/plic.h>
#include <sbi_utils/serial/sifive-uart.h>
#include <sbi_utils/sys/clint.h>

#define ROCKETCHIP_HART_COUNT 2
#define ROCKETCHIP_HART_STACK_SIZE 1024

#define ROCKETCHIP_SYS_CLK 50000000

#define ROCKETCHIP_CLINT_ADDR 0x2000000

#define ROCKETCHIP_PLIC_ADDR 0xc000000
#define ROCKETCHIP_PLIC_NUM_SOURCES 0x35
#define ROCKETCHIP_PLIC_NUM_PRIORITIES 7

#define ROCKETCHIP_UART_ADDR 0x10012000
#define ROCKETCHIP_UART_BAUDRATE 115200
#define INIT_SLOGAN "OpenSBI started!" 
...
static int platform_console_init(void)
{
    ...
    return sifive_uart_init(ROCKETCHIP_UART_ADDR, ROCKETCHIP_SYS_CLK,
                ROCKETCHIP_UART_BAUDRATE);
}
...
static int platform_irqchip_init(bool cold_boot)
{
    ...
    if (cold_boot) {
        ret = plic_cold_irqchip_init(ROCKETCHIP_PLIC_ADDR,
                        ROCKETCHIP_PLIC_NUM_SOURCES,
                        ROCKETCHIP_HART_COUNT);
        if (ret)
            return ret;
    }
...
static int platform_ipi_init(bool cold_boot)
{
    ...
    if (cold_boot) {
        ret = clint_cold_ipi_init(ROCKETCHIP_CLINT_ADDR,
                     	ROCKETCHIP_HART_COUNT);
        if (ret)
            return ret;
    }
...
static int platform_timer_init(bool cold_boot)
{
    ...
    /* Example if the generic CLINT driver is used */
    if (cold_boot) {
        ret = clint_cold_timer_init(ROCKETCHIP_CLINT_ADDR,
                        ROCKETCHIP_HART_COUNT, TRUE);
        if (ret)
            return ret;
    }
...
const struct sbi_platform platform = {
    ...
    .hart_count = ROCKETCHIP_HART_COUNT,
    .hart_stack_size = ROCKETCHIP_HART_STACK_SIZE,
