...
class RAS(nras: Int) {
  ...
  private val count = Reg(UInt(log2Up(nras + 1).W))
  private val pos = Reg(UInt(log2Up(nras).W))
...
class BHT(params: BHTParams)(implicit val p: Parameters) extends HasCoreParameters {
  def index(addr: UInt, history: UInt) = {
    ...
    hashAddr(addr) ^ (hashHistory(history) << (log2Up(params.nEntries) - params.historyBits)).asUInt
  }
  ...
  val history = Reg(UInt(params.historyLength.W))
}
...
class BTB(implicit p: Parameters) extends BtbModule {
  val io = new Bundle {
    val req = Input(Valid(new BTBReq))
    val resp = Output(Valid(new BTBResp))
    val btb_update = Input(Valid(new BTBUpdate))
    val bht_update = Input(Valid(new BHTUpdate))
    val bht_advance = Input(Valid(new BTBResp))
    val ras_update = Input(Valid(new RASUpdate))
    val ras_head = Output(Valid(UInt(vaddrBits.W)))
  ...
  private def pageMatch(addr: UInt) = {
    val p = page(addr)
    pageValid & pages.map(_ === p.asUInt()).asUInt
  }
  ...
  val nextPageRepl = Reg(UInt(log2Ceil(nPages).W))
  ...
  val samePage = page(r_btb_update.bits.pc).asUInt === page(update_target).asUInt
  ...
  when(r_btb_update.valid) {
    ...
    isValid := Mux(r_btb_update.bits.isValid, isValid | mask, isValid & (~mask).asUInt())
    if (fetchWidth > 1)
    ...
    writeBank(0, 2, Mux(idxWritesEven, idxPageReplEn, tgtPageReplEn),
      Mux(idxWritesEven, page(r_btb_update.bits.pc), page(update_target)).asUInt())
    writeBank(1, 2, Mux(idxWritesEven, tgtPageReplEn, idxPageReplEn),
      Mux(idxWritesEven, page(update_target), page(r_btb_update.bits.pc)).asUInt())
  ...
  io.resp.bits.mask := Cat((1.U << (~Mux(io.resp.bits.taken, ~io.resp.bits.bridx, 0.U)).asUInt()).asUInt() - 1.U, 1.U)
